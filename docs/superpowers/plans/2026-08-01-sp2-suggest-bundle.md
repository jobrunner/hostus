...
- Builds on SP1 (branch forked from `feature/sp1-foundation`): domain/ports/sqlite/wcvp/application/http/cli all present; `fts_name` (`unicode61 remove_diacritics 2`) + `fts_name_map` **exist in the schema but are NOT yet populated by the real ingest path** (SP1's parity test seeded them manually) — **T2 must wire FTS population into ingest** (one fts_name row per name linked to a concept — accepted + synonyms — mapping via `fts_name_map` to the concept, canonical [+ vernacular_de on the accepted]); a good approach is a `Finalize()`/`RebuildFTS` step on the ingest tx before Commit that `INSERT`s from `name JOIN concept_name`. `name.canonical_fold` = `domain.Canonicalize(canonical)`; ids = `<backbone>:concept|name:<taxonid>`; accepted ⇔ `wcvp.TaxonRow.IsAccepted()` self-reference; `distribution(area_scheme='wgsrpd_l3', area_code=<L3>)` populated from WCVP `locationid`.
