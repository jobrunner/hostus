#!/usr/bin/env bash
# GermanSL 1.5.5 (infinitenature.org) -> canonical name-list CSV.
#
# Source: https://germansl.infinitenature.org/downloads/ -> GermanSL 1.5 zip
# (GermanSL_1.5.5.zip), which bundles both the TURBOVEG export (species.dbf,
# ecodbase.dbf) and a spreadsheet export (GermanSL1.5.5.xlsx, sheet "TCS").
...
SOURCE_URL="https://germansl.infinitenature.org/GermanSL/1.5/GermanSL_1.5.5.zip"
SOURCE_ZIP="GermanSL_1.5.5.zip"
VERSION="1.5.5"
...
XLSX_PATH="${CACHE_DIR}/GermanSL1.5.5.xlsx"
...
if [[ ! -f "${XLSX_PATH}" ]]; then
  ...
  unzip -p "${ZIP_PATH}" "GermanSL 1.5/GermanSL1.5.5.xlsx" > "${XLSX_PATH}"
fi
